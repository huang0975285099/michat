...
- 速度系单体输出。
- EV：Lv.1 = 0.60，Lv.10 = 1.13。
- **档位变更：A→S**（rate 35→40、倍率 110%→150% 后 EV 已超过多数 S 档基准）。
...
- 双目标铺伤（原单体 120% 改为双目标各 100%）；倍率成长按群体档 0.03。
- EV：Lv.1 = 0.70，Lv.10 = 1.35。
- **档位变更：A→S**（单体改双目标后 EV 全表第二高，明显超出 A 档水准）。
...
- 三目标散射，对密集守军有效。
- EV：Lv.1 = 0.525，Lv.10 = 1.22。
- **档位变更：A→S**（rate 25→35 后 EV 超过落雷等既有 S 档基准）。
...
- 普攻后 40% 概率追加一次 100% 普攻伤害。
- EV：Lv.1 = 0.40，Lv.10 = 0.71。
...
- 唯一控制战法，跳过目标下一次行动（现为随机 2 名敌军）。
- durationScaleLevels=[10]（满级 +1 回合）。
- **档位变更：B→A**（目标 1→2 后控场覆盖面翻倍，B 档定价偏低）。
...
- **effect: heal**：行动前 30% 概率治疗随机 2 名我军，回复量按 casterPower 公式（施法者兵力 ×(1+智力/150)× 倍率 × 衰减系数）计算，受 maxLevel 成长。
- 设计参考：率土之滨「青囊秘要」、三国志战略版「刮骨疗毒」。
- 目标 `random_ally`，目标兵力未满时优先；超过上限取上限。
- 目标 1→2 后倍率成长改按群体档 0.03（原单体档 0.05），避免满级增速失控。
- 治疗不影响输出，与伤害战法 EV 不可直接比较，按"等效伤害"折算约 0.45；目标数翻倍后原折算基准需重新评估（粗算 raw EV：Lv.1=0.60，Lv.10=1.22，已明显高于凯歌等同类团队治疗战法，tier 是否仍适合 A 档建议后续复核）。
...
- **effect: buff**：行动前 35% 概率提升随机 1 名我军 50% 武力，持续 2 回合（满级 3 回合）。
- 设计参考：三国志战略版「陷阵营」「武锋阵」。
- buffDuration 持续按 lv 在 [5,10] 各 +1 回合。
- 增益以百分比叠加到 `atk` 属性后参与伤害计算。
- **档位变更：B→A**（增益数值 25%→50% 后是翻倍强度的团队增伤，B 档定价偏低）。
...
- **effect: buff**：行动前 35% 概率提升随机 1 名我军 50% 统率，持续 2 回合（满级 3 回合）。
- 增益以百分比叠加到 `def` 属性后参与减伤计算。
- **档位变更：B→A**（与激励同理，增益数值 25%→50% 后升档）。
...
- **damage + lifesteal**：行动前 40% 概率对随机 1 名敌军造成 100% 武力伤害，并将 30% 伤害回复为自身兵力。
- 设计参考：率土之滨「饮鸩止渴」、三国志战略版「暴戾无仁」（吸血变种）。
- EV（伤害部分）：Lv.1 = 0.40，Lv.10 = 0.84。吸血价值按 30% 折算约 +0.10~0.20。
...
- **extra_attack**：普通攻击后 40% 概率追加一次 150% 普攻伤害（受兵种克制影响）。
- 设计参考：率土之滨「鬼神霆威」、三国志战略版「一骑当千」（突击变种）。
- 概率已与连击拉平，靠更高倍率（150% vs 100%）体现差异。
- EV：Lv.1 = 0.60，Lv.10 = 1.00。
- **档位变更：A→S**（rate 25→40 后 EV 与疾风并列全表前段）。
...
|------|----|------|------|------|-----------|-----------|--------|------|------|
| 1 | `lipi` | 力劈 | 主动 | 武力 | 40% | 1.20 | 1 | 伤害 | 20 |
| 2 | `jifeng` | 疾风 | 主动 | 速度 | 40% | 1.50 | 1 | 伤害 | 25 |
| 3 | `huogong` | 火攻 | 主动 | 智力 | 35% | 1.00 | 2 | 伤害 | 30 |
| 4 | `jianyu` | 箭雨 | 主动 | 武力 | 35% | 0.50 | 3 | 伤害 | 30 |
| 5 | `luolei` | 落雷 | 主动 | 智力 | 30% | 0.50 | 3 | 伤害 | 30 |
| 6 | `lianji` | 连击 | 追击 | - | 40% | 1.00 | 1 | 追加普攻 | 20 |
| 7 | `huangbao` | 谎报 | 控制 | - | 30% | - | 2 | 跳过行动 | 20 |
| 8 | `qingnang` | 青囊 | 主动 | 智力 | 30% | 1.00 | 2（我军） | 治疗 | 30 |
...
| 13 | `shixue` | 嗜血 | 主动 | 武力 | 40% | 1.00 | 1 | 伤害+吸血 | 30 |
| 14 | `beishui` | 背水 | 主动 | 速度 | 40% | 1.00 | 1 | 伤害+残血爆发 | 25 |
| 15 | `guishen` | 鬼神 | 追击 | - | 40% | 1.50 | 1 | 追加普攻 | 30 |
