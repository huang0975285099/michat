...
- 文件分块传输：离线加密附件最大 500MB；旧版实时 WebSocket 文件通道保持 100MB
...
- 文件大小限制：离线加密附件为 500MB，前端和附件 HTTP 服务双层校验；兼容旧客户端的实时 WebSocket 文件通道仍限制 100MB，WebSocket 单帧上限 256KB，按 128KB 分块
