...
export default class TransitionScene extends Scene {
  ...
  playBonusMoves({ movesLeft = 0, bonusScore = 0 } = {}) {
    ...
    const glow = this.add.rectangle(width / 2, height / 2, width, height, 0xffc4df, 0.35)
    const title = this.add.text(width / 2, height / 2 - 34, 'Sweet Finish!', {
    ...
    const counter = this.add.text(width / 2, height / 2 + 34, `${movesLeft} moves  •  +0`, {
      fontFamily: 'Arial, sans-serif', fontSize: '20px', fontStyle: 'bold', color: '#66315d',
    }).setOrigin(0.5)
    this.root.add([glow, title, counter])
    ...
    return new Promise((resolve) => {
      this.tweens.addCounter({
        ...
        onUpdate: (tween) => {
          ...
          counter.setText(`${safeMoves - spent} moves  •  +${score}`)
        },
        onComplete: () => {
          counter.setText(`0 moves  •  +${bonusScore}`)
