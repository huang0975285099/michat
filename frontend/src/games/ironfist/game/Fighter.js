// Tekken - Phase 2 Procedural Vector Fighter (Phaser)
// Use the part textures generated by Graphics to build a Q version of the boxer, and all the animations are tween.
// Self-contained: If you change the sprite map in the future (after the third phase/material is in place), you only need to replace this file, and the BattleScene will remain unchanged.
// See docs/ironfist.md Section 22 Animation Evolution Route.

import Phaser from 'phaser'

// Shared textures (soft light/sparks), the entire game is only generated once
export function ensureFighterTextures(scene) {
  if (scene.textures.exists('if_glow')) return
  const g = scene.add.graphics()
  // Soft radial light (for halo/hit blast/shadow)
  for (let i = 26; i >= 0; i--) {
...
// Generate part texture for a certain side (named according to side to avoid cross-color between the two sides)
function makeParts(scene, side, pal, isRobot) {
  ...
  if (isRobot) {
    ...
    g.fillStyle(0x14223e, 1); g.fillRoundedRect(7, 15, 22, 9, 4)      //goggles
    g.fillStyle(pal.accent, 1); g.fillCircle(13, 19, 2.6); g.fillCircle(23, 19, 2.6)
    g.fillStyle(pal.bodyDark, 1); g.fillRect(16, 0, 4, 5)             //antenna
  } else {
    g.fillStyle(pal.head, 1); g.lineStyle(3, pal.bodyDark, 1)
    g.fillCircle(18, 18, 16); g.strokeCircle(18, 18, 16)
    g.fillStyle(0x232336, 1); g.fillCircle(12, 17, 2.6); g.fillCircle(24, 17, 2.6) //eye
    g.lineStyle(2, 0x232336, 1); g.beginPath(); g.arc(18, 24, 5, 0.15, Math.PI - 0.15); g.strokePath() //mouth
  }
  ...
  g.fillStyle(pal.accent, 1); g.fillRect(2, 28, 32, 5)               //Belt
  ...
  g.fillStyle(pal.gloveDark, 1); g.fillRoundedRect(9, 22, 8, 5, 2)   //wristband
...
export class Fighter {
   ...
   * @param {string} side 'me' | 'opp' (for texture key name and dodge direction)
   * @param {object} pal color matching
   * @param {number} dir direction towards the opponent (me=-1 upward, opp=+1 downward)
  ...
  // Attack posture (assuming you have returned to position at the beginning of the round)
  pose(name) {
  ...
  // Hit reaction: pushed back + white flash
  reactHit() {
    const s = this.scene, c = this.container, d = this.dir
  ...
  // End of round: animation return + clear temporary coloring
  resetPose() {
    const s = this.scene
