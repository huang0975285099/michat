...
export function createBattleRenderer3D(canvas, { playerCharged = false, opponentCharged = false, onReady } = {}) {
  ...
  // 环境光照（IBL）：用 studio.hdr 给 PBR 模型真实反射 + 柔和补光（背景仍保持暗调）。
  // 参数末位 prefilterOnLoad=true → 生成粗糙度反射所需的预过滤 mip。文件缺省时静默失败、退回纯灯光。
  const env = new BABYLON.HDRCubeTexture(GLB_ROOT + 'studio.hdr', scene, 128, false, true, false, true)
  scene.environmentTexture = env
  scene.environmentIntensity = 0.35

  // 灯光：环境 + 主光 + 蓝/红边缘光（呼应参考图左蓝右红）
  const hemi = new BABYLON.HemisphericLight('hemi', new BABYLON.Vector3(0, 1, 0), scene)
  hemi.intensity = 0.5  // IBL 已提供环境补光，半球光相应调低
  ...
  function _impact(r, meKO, oppKO) {
    ...
    if (r.playerDmg > 0) _impactFx(me, opp)
    if (r.opponentDmg > 0) _impactFx(opp, me)
  ...
  function _impactFx(victim, other) {
    ...
    sp.position.set(vx + dir * 0.35, 1.15, 0.18)
